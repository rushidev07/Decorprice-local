...
//            $transport = $transportBuilder->getTransport();
            ...
            if ($files) {
                ...
                foreach ($files as $file) {
                    ...
                    $filePath = $mediaPath . $this->_helperImage->getMediaPath('tmp'.$file['file']);
                    $attachPdf[] = $this->_transportBuilder->addAttachment($filePath, $file);
                }
    ...
    public function attachEmail($attachPdf)
    {
            ...
            $body              = Message::fromString($html->getRawMessage())->getBody();
            $body =    \Zend_Mime_Decode::decodeQuotedPrintable($body);

            $bodyMessage          = new Part($body);

            $bodyMessage->type    = 'text/html';
            $bodyMessage->charset = 'base64';
            $body = array_merge([$bodyMessage],$attachPdf);
            $bodyPart             = new \Zend\Mime\Message();
            $bodyPart->setParts($body);
