...
class Label extends Generic implements TabInterface
{
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset->addField('label', 'text', [
            'name' => 'label',
            'label' => __('Default Shipping Label for All Store Views'),
            'title' => __('Default Shipping Label for All Store Views'),
            'required' => true
        ]);

        if (!$this->_storeManager->isSingleStoreMode()) {
            $this->_addStoresFieldset($shippingLabel, $form);
        }
     ...
     * @throws LocalizedException
     */
    protected function _addStoresFieldset($shippingLabel, $form)
    {
        $labels = $shippingLabel ? $shippingLabel->getStoreLabels() : [];
