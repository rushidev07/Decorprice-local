...
class Ups extends Carrier implements CarrierInterface
{
    ...
    {
        return ObjectManager::getInstance()->get('Magento\Framework\App\Filesystem\DirectoryList');
    }
    ...
    public function requestLabel($track)
    {
        ...
        $request->setNode('Shipment/Package/Dimensions/UnitOfMeasurement/Code', $v->getDimensionUnits());
...
#$this->_logLoggerInterface->log($response);
    ...
    protected function _callShippingXml($call, $request)
    {
        ...
        #$this->_hlp->dump($baseUrl.'/'.$call, '__ups_call');
        #$this->_hlp->dump($request, '__ups_call');
        #$this->_hlp->dump($response, '__ups_call');
...
ITEMS: %items$s';
        ...
        foreach ($trackIds as $trackId) {
            ...
            try {
                ...
                foreach ($arr as $a) {
                    ...
                    if (in_array($status, ['M'/*Manifest Pickup*/, 'P'/*Pickup*/, 'X'/*Exception*/])) {
                        //record exists, but not picked up yet
                        continue;
                    }
                    if (in_array($status, ['D'/*delivered*/])) {
                        //delivered
                        $result[$trackId] = Source::TRACK_STATUS_DELIVERED;
                        continue;
                    }
                    if (in_array($status, ['I'/*In Transit*/])) {
                        //In Transit
    ...
    protected $_deliveryDatesCache=[];
    public function getDeliveryDateAll($vendor, $address, $weight, $value, $shipDate)
    {
        $reqKey = implode('-', [$vendor->getId(),$address->getId(),$weight,$value,$shipDate]);
