...
class Statement extends AbstractPdf
{
    ...
    protected $_globalTotals = [];
    protected $_globalTotalsAmount = [];
    ...
    public function isInPayoutAmount($amountType, $inPayoutOption, $vId = null)
    {
        ...
        $inPayoutOption = $inPayoutOption == 'include' ? ['', 'include'] : [$inPayoutOption];
    ...
    public function addStatement($statement)
    {
        ...
        $this->_curPageNum = 0;
        $this->addPage()->insertPageHeader(['first'=>true, 'data'=>$ordersData]);
    ...
    protected function _insertPageHeader($params = [])
    {
        $hlp = $this->_hlp;
        $store = null;
    ...
    public function insertPageHeader($params = [])
    {
        $hlp = $this->_hlp;
        $store = null;
        ...
        $this->_curPageNum++;
        $this->_pageFooter[] = ['page_num'=>$this->_curPageNum];
    ...
    public function insertAdjustmentsPageHeader($params = [])
    {
        $hlp = $this->_hlp;
        $store = null;
    ...
    public function insertPayoutsPageHeader($params = [])
    {
        $hlp = $this->_hlp;
        $store = null;
    ...
    public function insertRefundsPageHeader($params = [])
    {
        $hlp = $this->_hlp;
        $store = null;
    ...
    public function insertRefundsPage($params = [])
    {
        $hlp = $this->_hlp;

        $this->_totalsPageNum = 0;

        if (!$this->getStatement()->getRefunds()) {
            return $this;
        }
    ...
    public function insertTotals($totals)
    {
        ...
        foreach (['trans_fee','com_amount'] as $_k) {
    ...
    public function insertTotalsPageHeader()
    {
        ...
        $hideCnt = 0;
        foreach ([$hideTax, $hideShipping, $hideDiscount] as $hideIdx) {
            $hideCnt += (int)$hideIdx;
        }
    ...
    public function insertTotalsPage()
    {
        ...
        $totals = ['subtotal'=>0, 'tax'=>0, 'shipping'=>0, 'discount' => 0, 'handling'=>0, 'com_amount'=>0, 'trans_fee'=>0, 'adj_amount'=>0, 'total_payout'=>0, 'total_paid'=>0, 'total_due'=>0, 'total_refund'=>0];
        if (!$this->_hlp->isStatementAsInvoice()) {
            $totals = array_merge($totals, ['total_payout'=>0, 'total_paid'=>0, 'total_due'=>0, 'total_refund'=>0]);
        } else {
            $totals = array_merge($totals, ['total_invoice'=>0, 'total_refund'=>0]);
        }
        ...
        foreach ($this->_globalTotals as $vId => $line) {
            ...
            foreach ($totals as $k => &$v) {
                if ($k == 'shipping' && $this->isInPayoutAmount('shipping', 'include', $vId)
                    ...
                    || !in_array($k, ['tax','shipping','discount'])
                ) {
    ...
    public function insertAdjustmentsPage($params = [])
    {
        $hlp = $this->_hlp;

        $this->_totalsPageNum = 0;
        
        if (!$this->getStatement()->getExtraAdjustments()) {
            return $this;
        }
        ...
        foreach ($this->getStatement()->getExtraAdjustments() as $line) {
            foreach (['adjustment_id','po_id','amount','comment','username','po_type','created_at'] as $_k) {
                if (!isset($line[$_k])) {
                    $line[$_k] = '';
                }
            }
    ...
    public function insertPayoutsPage($params = [])
    {
        $hlp = $this->_hlp;

        $this->_totalsPageNum = 0;
        
        if (!$this->getStatement()->getPayouts()) {
            return $this;
        }
        ...
        foreach ($this->getStatement()->getPayouts() as $line) {
            foreach (['payout_id','payout_type','payout_method','payout_status','total_orders','total_payout','total_paid','total_due'] as $_k) {
                if (!isset($line[$_k])) {
                    $line[$_k] = '';
                }
            }
