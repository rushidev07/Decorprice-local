...
window.UnirgyModal = Class.create({
    initialize: function() {
    ...
    showModal: function(newModal, skipNextClick, mode) {
        ...
        */
        ...
        if ((/msie 7\./i).test(navigator.userAgent)) {
            this.curModal.select('.umodal-main')[0].setStyle({
                height: this.curModal.select('.umodal-main .content')[0].getHeight()+'px'
            })
        }
        */
        /*
        var modalHeight = Math.max(
            this.curModal.select('.umodal')[0].getHeight(),
            this.curModal.select('.umodal-main')[0].getHeight(),
            this.curModal.select('.umodal-main .content')[0].getHeight()
        ).round()
        */
        var modalHeight = this.curModal.select('.umodal')[0].getHeight();
        this.curModal.down().absolutize()
        /*this.curModal.down().setStyle({
            zIndex: 10000,
            left: (
                document.viewport.getScrollOffsets().left
                +(document.viewport.getWidth()-this.curModal.select('.umodal-main')[0].getWidth())/2).round()+'px',
            top: (
        ...
        var showStyle = {
            ...
            left: (
                document.viewport.getScrollOffsets().left
                +(myWidth-this.curModal.select('.umodal-main')[0].getWidth())/2).round(),
            top: (
                document.viewport.getScrollOffsets().top
        ...
        if (Object.isArray(mode) && mode.length>=2 && mode[0] && $(mode[1])) {
            if (mode[0]&this.center) {
                showStyle.left = $(mode[1]).cumulativeOffset().left
                    +(($(mode[1]).getWidth()-this.curModal.select('.umodal')[0].getWidth())/2).round();
            }
            if (mode[0]&this.left) {
                showStyle.left = $(mode[1]).cumulativeOffset().left
                    -this.curModal.select('.umodal')[0].getWidth();
            }
            ...
            if (mode[0]&this.middle) {
                showStyle.top = $(mode[1]).cumulativeOffset().top
                    +(($(mode[1]).getHeight()-this.curModal.select('.umodal')[0].getHeight())/2).round();
            }
            if (mode[0]&this.top) {
                showStyle.top = $(mode[1]).cumulativeOffset().top
                    -this.curModal.select('.umodal')[0].getHeight();
...
window.unirgyGetModal = function() {
    if (!window.UnirgyModalSingleton) {
        window.UnirgyModalSingleton = new UnirgyModal();
    }
    return window.UnirgyModalSingleton;
};

window.unirgyShowThisModal = function(modalId, mode) {
    if ($(modalId)) {
        unirgyGetModal().showModal($(modalId), true, mode)
    }
};

window.unirgyShowThisSizeChartModal = function(scmId, mode) {
    if ($(scmId)) {
        unirgyGetModal().showModal($(scmId), true, mode)
    }
};

window.unirgyShowSizeChartModal = function(mode) {
    if ($('size-chart-modal')) {
        unirgyGetModal().showModal($('size-chart-modal'), true, mode)
    }
};
return window.unirgyGetModal;
}
);
