...
class MassUpload extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if(!$this->dataHelper->checkForConfiguration()) {
            $this->messageManager->addErrorMessage(__('Products Upload Failed. Houzz API not enabled or Invalid. Please check Houzz Configuration.'));
        ...
        if (isset($batchid)) {
            ...
            if (isset($productids[$batchid]) && $this->dataHelper->createProductOnHouzz($productids[$batchid])) {
                return $resultJson->setData([
                    'success' => "<b> SKU : " . $product->getSku() . "</b> Product(s) Uploaded successfully",
                ]);
            }
            return $resultJson->setData([
                'error' => "<b> SKU : " . $product->getSku() . "</b> Product(s) Upload Failed",
            ]);
        }
        ...
        if (count($productids) < self::CHUNK_SIZE - 1) {
            if ( $this->dataHelper->createProductOnHouzz($productids)) {
