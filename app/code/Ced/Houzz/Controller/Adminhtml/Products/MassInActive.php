...
class MassInActive extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if (count($productids) == 0) {
            $this->messageManager->addErrorMessage('No Product selected for Inactive On Houzz.');
            $this->_redirect('houzz/products/index');
        }
        $response=$this->dataHelper->changeProductStatusOnHouzz($productids , "inactive");
        $countsuccess=0;
        $counterror=0;
        if(!empty($response)) {
            ...
            if (!empty($response)) {
                $this->messageManager->addSuccessMessage("Total " . $countsuccess . ' Product(s) Inactive On Houzz Successfully.');

            }
            $this->messageManager->addErrorMessage("Total " . $counterror . ' Product(s) Inactive On Houzz Failed.');
