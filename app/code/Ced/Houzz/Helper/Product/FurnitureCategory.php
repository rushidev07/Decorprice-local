...
class FurnitureCategory extends \Ced\Houzz\Helper\Product\Base
{
    /**
    ...
    public function setData(
        ...
        if (!empty($product) && !empty($attributes) && !empty($category)) {
            ...
            switch ($category['cat_id']) {
                case 'Furniture' : {
                    $data['Furniture'] =
                        $this->setFurniture($product, $attributes);
                    break;
                }
            }
        }
     ...
     * @return string|array()
     */
    public function setFurniture($product = array(), $attributes = array())
    {
        $houzzAttr = array(
            'pieceCount','shortDescription','keyFeatures','brand','manufacturer','modelNumber',
            'manufacturerPartNumber','multipackQuantity','countPerPack','count','mainImageUrl',
            'productSecondaryImageURL','color','colorCategory/colorCategoryValue','finish','material',
            'pattern','gender','ageGroup/ageGroupValue','size','bedSize','homeDecorStyle',
            'seatingCapacity','numberOfDrawers','numberOfShelves','collection','theme','shape',
            'diameter/measure','diameter/unit','bedStyle','mountType','assembledProductLength/measure',
            'assembledProductLength/unit','assembledProductWidth/measure','assembledProductWidth/unit',
            'assembledProductHeight/measure','assembledProductHeight/unit','assembledProductWeight/measure',
            'assembledProductWeight/unit','variantGroupId','variantAttributeNames/variantAttributeName',
            'isPrimaryVariant','isProp65WarningRequired','prop65WarningText','requiresTextileActLabeling',
            'countryOfOriginTextiles','hasWarranty','warrantyURL','warrantyText',
            'isLightingFactsLabelRequired','lightingFactsLabel','isAssemblyRequired','assemblyInstructions',
            'fabricContent','fabricCareInstructions','fabricColor','accentColor','cushionColor','recommendedUses',
            'recommendedRooms','recommendedLocations','mattressFirmness','mattressThickness/measure',
            'mattressThickness/unit','pumpIncluded','fillMaterial','frameMaterial','seatMaterial',
            'seatHeight/measure','seatHeight/unit','seatBackHeight/measure','seatBackHeight/unit',
            'tableHeight/measure','tableHeight/unit','topMaterial','topDimensions','topFinish',
            'hardwareFinish','baseMaterial','baseColor','baseFinish','doorOpeningStyle','doorStyle',
            'slatWidth/measure','slatWidth/unit','numberOfHooks','headboardStyle','frameColor',
            'isAntique','isFoldable','isInflatable','isWheeled','isIndustrial','isPowered','powerType',
            'character','globalBrandLicense','sportsLeague','sportsTeam','athlete','autographedBy',
            'features','keywords','numberOfPanels','seatBackStyle',
