...
class CarriersAndAccessoriesCategory extends \Ced\Houzz\Helper\Product\Base
{
    /**
        ...
        if (!empty($product) && !empty($attributes) && !empty($category)) {
            switch ($category['cat_id']) {
                ...
                case 'CarriersAndAccessories' : {
                    $data['CarriersAndAccessories'] = $this->setCarriersAndAccessories($product, $attributes);
                    break;
                }
            }
        }
     ...
     * @return string|array()
     */
    public function setCasesAndBags($product = array(), $attributes = array())
    {
        $houzzAttr = array(
            ...
            'color','colorCategory','colorCategory/colorCategoryValue','pattern','material','gender','size','ageGroup',
            'ageGroup/ageGroupValue','capacity','numberOfWheels','hardOrSoftCase','isWheeled','compatibleBrands',
            'compatibleDevices','screenSize','screenSize/measure','screenSize/unit','bagStyle','assembledProductLength',
            'assembledProductLength/measure','assembledProductLength/unit','assembledProductWidth',
            'assembledProductWidth/measure','assembledProductWidth/unit','assembledProductHeight',
            'assembledProductHeight/measure','assembledProductHeight/unit','assembledProductWeight',
            'assembledProductWeight/measure','assembledProductWeight/unit','variantGroupId','variantAttributeNames',
            'variantAttributeNames/variantAttributeName','isPrimaryVariant','isProp65WarningRequired',
            'prop65WarningText','batteryTechnologyType','hasWarranty','warrantyURL','warrantyText',
            'requiresTextileActLabeling','countryOfOriginTextiles','fabricContent','fabricCareInstructions',
            'shape','maximumWeight','maximumWeight/measure','maximumWeight/unit','finish','numberOfCompartments',
            'hasRemovableStrap','designer','character','handleMaterial','handleType','zipperMaterial',
            'lockingMechanism','leatherGrade','isWeatherResistant','isWaterproof','fastenerType','sport',
            'sportsLeague','sportsTeam','athlete','autographedBy','features','keywords','monogramLetter',
            'isReusable','isFoldable','globalBrandLicense','isLined','recommendedUses','occasion',
            'isMadeFromRecycledMaterial','recycledMaterialContent','swatchImages','swatchImages/swatchImage',
     ...
     * @return string|array()
     */
    public function setCarriersAndAccessories($product = array(), $attributes = array())
    {
        $houzzAttr = array(
            'shortDescription','keyFeatures','brand','manufacturer','manufacturerPartNumber','modelNumber',
            'multipackQuantity','countPerPack','count','pieceCount','mainImageUrl','productSecondaryImageURL',
