...
namespace Ced\Houzz\Model\ResourceModel;
...
class Profileproducts extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function _construct()
    {
        ...
        $this->_init('houzz_profile_products', 'id');
    ...
    public function deleteFromProfile($productId)
    {
        ...
        $condition = "{$this->getTable('houzz_profile_products')}.product_id = " . $dbh->quote($productId);
        //. " AND {$this->getTable('houzz/profileproducts')}.profile_id = " . $dbh->quote($profileId);
        $dbh->delete($this->getTable('houzz_profile_products'), $condition);
        return $this;
    ...
    {
        if ( $productId > 0 ) {
            $profileTable = $this->getTable('houzz_profile_products');

            $productProfile = \Magento\Framework\App\ObjectManager::getInstance()->get('Ced\Houzz\Model\Profileproducts')->loadByField('profile_id',$profileId);
