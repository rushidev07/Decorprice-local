...
class OrderInfoBuilder implements OrderInfoBuilderInterface
{
    ...
    public function gatherInfo($order, $existingOrderData)
    {
        ...
        $wsOrderData = $this->mapFieldsForWeSupplyOrderStructure($orderData, $existingOrderData);

        $this->setNsgEpsiData($order, $wsOrderData);
    ...
    protected function mapOrderDataToWeSupply($orderData, &$finalOrderData)
    {
        ...
        $baseUrl = $this->storeManagerInterface->getStore($finalOrderData['StoreId'])->getBaseUrl();
        $parsedUrl = parse_url($baseUrl);
        $finalOrderData['StoreDomain'] = $parsedUrl['host'];
     ...
     * @param $order
     */
    private function setNsgEpsiData($order, &$wsOrderData)
    {
        $wsOrderData['IsEpsi'] = $order->getIsEpsi() ?? 0;
        $wsOrderData['EpsiAmount'] = $order->getEpsiAmount() ?? 0;

        $creditMemos = $order->getCreditmemosCollection();

        foreach ($creditMemos->getData() as $creditmemo) {
            if (!isset($creditmemo['is_epsi']) && $creditmemo['epsi_amount']) {
                continue;
            }

            if ($creditmemo['is_epsi']
                && $wsOrderData['EpsiAmount'] > 0 // avoid negative values
            ) {
                $wsOrderData['EpsiAmount'] -= floatVal($creditmemo['epsi_amount']);
            }
        }

        if (!$wsOrderData['EpsiAmount']) {
            // Just make sure the IsEpsi is set to 0 if the EpsiAmount is 0
            $wsOrderData['IsEpsi'] = 0;
        }
    }
}
