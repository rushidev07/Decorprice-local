...
class PhpScanner implements ScannerInterface
{
        ...
        foreach ($files as $file) {
            ...
            foreach ($classes as $className) {
                try {
                    ...
                    // Skip classes that cannot be reflected (e.g. SourceGuardian-encoded dependencies)
                    continue;
                }
    ...
    protected function _fetchNamespace($tokenIterator, $count, $tokens)
    {
        ...
        for ($tokenOffset = $tokenIterator + 1; $tokenOffset < $count; ++$tokenOffset) {
            ...
            } elseif (is_array($tokens[$tokenOffset])
                && ($tokens[$tokenOffset][0] === T_NAME_QUALIFIED
                    || $tokens[$tokenOffset][0] === T_NAME_FULLY_QUALIFIED)
            ) {
                // PHP 8.0+ tokenizes full namespace paths as a single T_NAME_QUALIFIED token
                $namespaceParts[] = "\\";
                $namespaceParts[] = $tokens[$tokenOffset][1];
